...
worker_image="$(read_file_value "$manifest" WORKER_IMAGE)"
...
awk -F= \
  ...
  -v revision="$revision" '
    ...
    $1 == "WORKER_IMAGE" { print "WORKER_IMAGE=" worker; seen_worker=1; next }
    ...
    END {
      ...
      if (!seen_worker) print "WORKER_IMAGE=" worker
