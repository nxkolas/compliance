...
export function DashboardActionLink({ href, label, draftKey, startLabel, continueLabel, className }: { href: string; label: string; draftKey?: string; startLabel?: string; continueLabel?: string; className?: string }) {
  const draft = useSyncExternalStore(subscribeApplicabilityDraft, () => draftKey ? readApplicabilityDraft(draftKey) : null, () => null);
  return <Link href={href} className={cn("mt-5 ml-auto flex w-fit max-w-full items-center justify-end gap-1 font-sans text-[10px] leading-5 font-medium text-white/70 hover:text-white focus-visible:outline-2 focus-visible:outline-offset-4", className)}>
