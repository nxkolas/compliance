    ...
    <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent>
        ...
        <div className="grid gap-4">
          ...
          <div className="grid gap-2"><Label>{labels.aiPolicy}</Label><Select value={form.aiProviderMode} onValueChange={(value) => setForm({ ...form, aiProviderMode: value as SerializedItem["aiProviderMode"] })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="company_hosted">{labels.providerModes.company_hosted}</SelectItem><SelectItem value="openai">{labels.providerModes.openai}</SelectItem><SelectItem value="self_hosted">{labels.providerModes.self_hosted}</SelectItem></SelectContent></Select></div>
        </div>
