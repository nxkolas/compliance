...
import { validateEmbeddings } from "@/src/server/platform/content-processing";
import type { ContentEmbedder } from "@/src/server/platform/content-processing";
import { organizationEmbeddingProvider } from "@/src/server/modules/documents";
import { emitGenerationMetric } from "../../platform/ai/generation/metrics";
  ...
  >({
    ...
    async generate({
      ...
      const grounded = await runGroundedOperation<GapCategoryResponse>({
        ...
        outputLocaleInstruction: gapOutputLocaleInstruction(input.outputLocale),
        systemInstruction:
          phase === "initial"
            ? gapPrompt({
                locale: input.outputLocale,
...
async function prepareQueryEmbeddings(
  ...
  const queries = includeOrganizationDocuments
    ...
    : [];
  if (!embeddingProvider) throw new Error("Document embedding provider is required");
