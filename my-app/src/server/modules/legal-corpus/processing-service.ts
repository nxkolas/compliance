...
import { paragraphContentChunker } from "@/src/server/platform/content-processing/defaults";
import { parseDocument } from "@/src/server/platform/content-processing/parser";
import type { ContentChunker, ContentParser } from "@/src/server/platform/content-processing/types";
import { MAX_LEGAL_SOURCE_BYTES } from "./config";
...
export async function executeLegalSourceProcessingJob(
  ...
  const parser = dependencies.parser ?? {
    parse: (bytes: Uint8Array, mimeType: string) =>
      parseDocument(bytes, mimeType, { maxBytes: MAX_LEGAL_SOURCE_BYTES }),
  };
