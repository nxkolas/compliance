...
import { activeComplianceCheckReleases, complianceCheckReleaseActivations } from "@/src/db/schema";
...
export async function activateComplianceRelease(
  ...
  return db.transaction(async (tx) => {
    ...
    const [ruleSet, questionnaireVersion, scopeModelVersion, thresholdSet, profileLinks, factLinks, contentLinks, corpusPins] = await Promise.all([
      tx.query.ruleSets.findFirst({ columns: { id: true, moduleId: true, code: true, versionLabel: true, status: true, evaluatorKind: true, evaluatorSchemaVersion: true, rules: true, contentHash: true, createdAt: true, publishedAt: true }, where: { RAW: (table, operators) => (eq(table.id, release.ruleSetId)) ?? operators.sql`true` } }),
      tx.query.questionnaireVersions.findFirst({ columns: { id: true, questionnaireId: true, versionLabel: true, titleContentRevisionId: true, status: true, createdAt: true, publishedAt: true }, where: { RAW: (table, operators) => (eq(table.id, release.questionnaireVersionId)) ?? operators.sql`true` } }),
      tx.query.scopeModelVersions.findFirst({ columns: { id: true, scopeModelId: true, versionLabel: true, status: true, effectiveFrom: true, effectiveTo: true, contentHash: true, publishedAt: true }, where: { RAW: (table, operators) => (eq(table.id, release.scopeModelVersionId)) ?? operators.sql`true` } }),
      tx.query.scopeThresholdSets.findFirst({ columns: { id: true, code: true, versionLabel: true, status: true, mediumEmployeeThreshold: true, mediumTurnoverThreshold: true, mediumBalanceSheetThreshold: true, largeEmployeeThreshold: true, largeTurnoverThreshold: true, largeBalanceSheetThreshold: true, employeeComparison: true, financialComparison: true, contentHash: true, publishedAt: true }, where: { RAW: (table, operators) => (eq(table.id, release.scopeThresholdSetId)) ?? operators.sql`true` } }),
      tx.query.complianceCheckReleaseProfiles.findMany({ columns: { checkReleaseId: true, countryCode: true, jurisdictionProfileVersionId: true }, where: { RAW: (table, operators) => (eq(table.checkReleaseId, release.id)) ?? operators.sql`true` } }),
      tx.query.complianceCheckReleaseFactVersions.findMany({ columns: { checkReleaseId: true, factDefinitionVersionId: true }, where: { RAW: (table, operators) => (eq(table.checkReleaseId, release.id)) ?? operators.sql`true` } }),
      tx.query.complianceCheckReleaseContentRevisions.findMany({ columns: { checkReleaseId: true, contentRevisionId: true }, where: { RAW: (table, operators) => (eq(table.checkReleaseId, release.id)) ?? operators.sql`true` } }),
      tx.query.complianceCheckReleaseCorpusReleases.findMany({ columns: { checkReleaseId: true, familyId: true, corpusReleaseId: true }, where: { RAW: (table, operators) => (eq(table.checkReleaseId, release.id)) ?? operators.sql`true` } }),
    ]);
    ...
    const [profileVersions, nationalVersions, effectiveStates] = profileVersionIds.length > 0
      ? await Promise.all([
          tx.query.jurisdictionProfileVersions.findMany({ columns: { id: true, jurisdictionProfileId: true, versionLabel: true, status: true, supported: true, allowNegativeConclusion: true, effectiveFrom: true, effectiveTo: true, contentHash: true, publishedAt: true }, where: { RAW: (table, operators) => (inArray(table.id, profileVersionIds)) ?? operators.sql`true` } }),
          tx.query.jurisdictionEntityTypeVersions.findMany({ columns: { id: true, jurisdictionEntityTypeId: true, jurisdictionProfileVersionId: true, statutoryCategoryCode: true, annex: true, classificationRule: true, labelContentRevisionId: true, descriptionContentRevisionId: true, definitionHash: true }, where: { RAW: (table, operators) => (inArray(table.jurisdictionProfileVersionId, profileVersionIds)) ?? operators.sql`true` } }),
          tx.query.jurisdictionProfileEffectiveStates.findMany({ columns: { id: true, jurisdictionProfileVersionId: true, code: true, stateValue: true, effectiveFrom: true, effectiveTo: true, reviewedAt: true, officialSourceUrl: true, legalProvisionId: true, declarationHash: true }, where: { RAW: (table, operators) => (inArray(table.jurisdictionProfileVersionId, profileVersionIds)) ?? operators.sql`true` } }),
        ])
      : [[], [], []];
    const nationalVersionIds = nationalVersions.map((version) => version.id);
    const nationalMappings = nationalVersionIds.length > 0
      ? await tx.query.jurisdictionEntityTypeMappings.findMany({ columns: { id: true, jurisdictionEntityTypeVersionId: true, scopeEntityTypeId: true, relationshipKind: true }, where: { RAW: (table, operators) => (inArray(table.jurisdictionEntityTypeVersionId, nationalVersionIds)) ?? operators.sql`true` } })
      : [];
    ...
    const current = await tx.query.activeComplianceCheckReleases.findFirst({ columns: { checkCode: true, checkReleaseId: true, activatedBy: true, activatedAt: true },
      where: { RAW: (table, operators) => (eq(table.checkCode, checkCode)) ?? operators.sql`true` },
    });
