...
} from "@/src/server/platform/ai/generation/failures";
import type { OrganizationCapability } from "../platform/auth/capabilities";
...
const payloadSchemas = {
  ...
  action_plan_generation: z.object({
    ...
    locale: z.enum(["de", "en"]),
    gapDefinitionHash: z.string().min(1),
    actionPlanDefinitionHash: z.string().min(1),
    buildHash: z.string().min(1),
  }).strict(),
...
export const jobDefinitions = {
  gap_analysis: {
    ...
    async execute(job, payload, signal) {
      const { executeGapGenerationJob } = await import("../modules/gap-analysis");
  ...
  gap_conflict_resolution: {
    ...
    resultSchema: locator("analysis_output_revision"),
    async execute(job, payload, signal) {
      const { executeGapContradictionResolutionJob } = await import("../modules/gap-analysis");
  ...
  action_plan_generation: {
    ...
    async execute(job, payload, signal) {
      const { executeActionPlanGenerationJob } = await import("../modules/action-plans");
  ...
  report_render: {
    ...
    async execute(job, payload, signal) {
      const { handleReportRender } = await import("../modules/reports");
  ...
  document_indexing: {
    ...
    async execute(job, payload) {
      const { executeDocumentIndexingJob } = await import("../modules/documents");
  ...
  legal_source_processing: {
    ...
    async execute(job, payload, signal) {
      const { executeLegalSourceProcessingJob } = await import("../modules/legal-corpus");
  ...
  maintenance_cleanup: {
    ...
    async execute(job) {
      const { runMaintenanceCleanup } = await import("./maintenance");
