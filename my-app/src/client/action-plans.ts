...
export const actionPlansClient = {
  ...
  updateItem(organizationId: string, itemId: string, input: z.input<typeof actionPlanItemUpdateSchema>) {
    return request(`${base(organizationId)}/items/${encodeURIComponent(itemId)}`, { method: "PATCH", input: actionPlanItemUpdateSchema.parse(input), outputSchema: z.object({ item: actionPlanItemSchema }) });
  },
};
