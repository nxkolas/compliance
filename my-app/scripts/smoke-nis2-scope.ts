...
async function main() {
  try {
    ...
    const questionRows = await sql<{ id: string; stableKey: string }[]>`
      select id, stable_key as "stableKey"
      from questions
      order by position
    `;
    ...
    const fixtures = [
      { ...baseFacts, nis2_entity_types: ["dns_service_provider"] },
      {
        ...baseFacts,
    ...
    for (const fixture of fixtures) {
      ...
      const submission = await submitApplicabilityCheckForGuest({ answers });
      smokeGuestIds.push(submission.id);
    ...
    if (
      parsedRuleSet.kind !== "nis2_scope_v2" ||
