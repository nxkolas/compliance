...
async function main() {
  ...
  for (const code of expectedFamilies) {
    ...
    const evaluation = await db.query.legalCorpusEvaluations.findFirst({
      where: and(
  ...
  const compliancePointer = await db.query.activeComplianceCheckReleases.findFirst({
  ...
  const complianceRelease = await db.query.complianceCheckReleases.findFirst({
  ...
  const compliancePins = await db.query.complianceCheckReleaseCorpusReleases.findMany({
  ...
  const gapPointer = await db.query.activeGapAnalysisReleases.findFirst({
  ...
  const gapPins = await db.query.gapAnalysisReleaseCorpusReleases.findMany({
    where: eq(gapAnalysisReleaseCorpusReleases.gapAnalysisReleaseId, gapRelease.id),
  });
  assertExactPins("Gap", gapPins, corpusRows);

  const unfinishedJobs = await db.query.backgroundJobs.findMany({
    where: or(
  ...
  for (const eventType of [
    ...
    const event = await db.query.platformAuditEvents.findFirst({
