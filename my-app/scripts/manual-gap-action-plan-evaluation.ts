...
import type { GapAnswerValue } from "@/src/server/gap-analysis/deterministic-evaluator";
...
async function main() {
  ...
  if (resumeCaseFiveOrganizationId) {
    ...
    console.log(
      `Completed resumed case 5 and manifest. Raw output: ${OUTPUT_DIR}`,
    );
    return;
  }
  ...
  for (const testCase of cases) {
    console.log(`\n[${testCase.number}/5] ${testCase.title}`);
    ...
    console.log(
      `Saved case ${testCase.number}: ${result.organization.name} (${result.organization.id})`,
    );
  }

  await writeManifest(results, runStartedAt);
  console.log(`\nCompleted five-case evaluation. Raw output: ${OUTPUT_DIR}`);
}
...
async function executeCase(testCase: EvaluationCase) {
  ...
  if (testCase.manualCorrection) {
    ...
    const blockers = generated.findings.filter(
    ...
    if (blockers.length) {
      const correctedRevision = await correctGapRevision({
        ...
        organizationId: organization.id,
        sourceRevisionId: completedDraft.outputGapRevisionId,
        corrections: blockers.map((finding) => ({
          findingId: finding.id,
          requiresReview: false,
          reason:
            "Manual QA reviewer cleared an unexpected model review blocker to test action-plan materialization.",
          resolutionReason:
            "No organization document was supplied, so the generated questionnaire-only finding was accepted for this synthetic fixture.",
        })),
      });
      finalRevisionId = correctedRevision.id;
    }
...
async function resumeCaseFive(organizationId: string) {
  ...
  const fullyResolved = await correctGapRevision({
    userId: USER_ID,
    organizationId,
    ...
    corrections: blockers.map((finding) => ({
      ...
      resolutionReason:
        finding.requirementCode === "NIS2-ASSURE-08"
  ...
  return {
    ...
    workflow: {
      ...
      finalRevisionId: fullyResolved.id,
      correction: {
        requirementCode: "NIS2-BC-05",
        status: "not_fulfilled",
        evidenceSufficiency: "sufficient",
...
function buildAutomaticChecks(input: {
  ...
  for (const item of input.plan.items) {
    ...
    checks.push({
      name: `${item.requirementCode} action description`,
      passed:
        item.description === source?.recommendation &&
        item.sourceFindingRecommendation === source?.recommendation,
      expected: source?.recommendation ?? null,
