...
export default async function PdfExportPage({ params }: { params: Promise<{ organizationId: string }> }) {
  ...
  return <section className="flex w-full min-w-0 flex-col gap-6 sm:gap-8"><PageHeader title={dictionary.modules.pdfExport.title} subtitle={dictionary.modules.pdfExport.description} /><ReportWorkflow organizationId={organizationId} locale={locale} reports={reports} canCreate={hasOrganizationCapability(membership.role, "reports:create")} labels={dictionary.reports.workflow} /></section>;
}
