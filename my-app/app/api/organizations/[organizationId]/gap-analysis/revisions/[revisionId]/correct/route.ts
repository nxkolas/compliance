...
export const POST = apiRoute(async ({ request, routeContext }: { request: Request; routeContext: { params: Promise<{ organizationId: string; revisionId: string }> } }) => {
  ...
  const result = await runIdempotentCommand({ repository: databaseIdempotencyRepository, request, actorKey: user.id, scope: organizationId, operation: "gap-revision.correct", requestInput: { sourceRevisionId: revisionId, ...body }, resultType: "generated_artifact_revision", responseStatus: 201, execute: () => correctGapRevision({ userId: user.id, organizationId, sourceRevisionId: revisionId, ...body }), resultId: (revision) => revision.id, replay: async (id) => { const value = await getGapAnalysisRevision({ userId: user.id, organizationId, revisionId: id }); if (!value) throw new ApiError(409, "Corrected revision is unavailable", undefined, "IDEMPOTENCY_RESULT_UNAVAILABLE"); return value.revision; } });
