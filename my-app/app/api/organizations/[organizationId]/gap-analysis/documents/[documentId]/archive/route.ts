...
export async function POST(_: Request, context: { params: Promise<{ organizationId: string; documentId: string }> }) {
  try {
    const user = await requireApiUser();
    const { organizationId, documentId } = await context.params;
    const document = await archiveOrganizationDocument(user.id, organizationId, documentId);
