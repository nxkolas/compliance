...
export const POST = apiRoute(async ({ request, routeContext }: { request: Request; routeContext: { params: Promise<{ organizationId: string; invitationId: string }> } }) => {
  ...
  const result = await runIdempotentCommand<OrganizationInvitationDto>({ repository: databaseIdempotencyRepository, request, actorKey: user.id, scope: params.organizationId,
